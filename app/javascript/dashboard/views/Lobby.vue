  ...
  <div>
    <div class="grid grid-cols-1 lg:grid-cols-2 w-full h-full">
      <div class="flex flex-col items-center overflow-y-auto">
        <div class="mb-auto w-full py-4 md:py-8 md:px-8 px-4 flex justify-between items-start">
          <a href="https://scalechamp.com" class="block">
            <Logo class="hidden md:inline-block h-8 fill-current text-primary"/>
            <Loading class="inline-block md:hidden h-8 fill-current text-primary" />
          </a>
          <div class="group relative block z-10">
            <div class="cursor-pointer whitespace-no-wrap  text-gray-800">
              <svg class="inline w-6 h-6 fill-current" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 477.73 477.73" style="enable-background: new 0 0 477.73 477.73;" xml:space="preserve">
                <g>
                  <g>
              ...
              <svg class="inline fill-current h-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 255 255">
                <path d="M0 63.75l127.5 127.5L255 63.75z" />
              </svg>
            </div>
            <div class="hidden group-hover:block pt-2 absolute top-full right-0 text-left">
              <div class="rounded-sm bg-white text-black font-semibold py-2 shadow border border-gray-200">
                <span v-for="locale in ['en', 'ru', 'uk']"
                      class="cursor-pointer block px-4  py-1"
                      :class="{ 'hover:bg-gray-200': locale !== $i18n.locale, 'bg-primary hover:bg-primary-100 text-white': locale === $i18n.locale }"
                      :key="locale"
        ...
        <router-view class="m-auto px-4 sm:w-5/6 md:w-full md:max-w-lg space-y-4" />
        <div class="mt-auto py-8 px-4 space-y-8">
          <div class="flex flex-col sm:flex-row items-center space-x-1 w-full">
              <p class="block text-gray-700">{{ footerText.title }}</p>
              <router-link class="block text-primary hover:text-primary-100" :to="{ name: footerText.to }">{{ footerText.info }}</router-link>
          </div>
          <small class="block text-center tracking-wide text-gray-500 text-sm">
            {{ $t('copyright_full') }}
          </small>
        </div>
      </div>
      <div class="bg-gray-900 hidden lg:block pixel-sorting bg-center bg-cover" />
    </div>
  </div>
</template>
  ...
  computed: {
    footerText () {
      switch (this.$route.name) {
        case 'login':
          return {
            title: this.$t('lobby.account_missing'),
            info: this.$t('lobby.create_account'),
            to: 'register'
          }
        case 'register':
          return {
            title: this.$t('lobby.account_exists'),
            info: this.$t('lobby.or_login'),
            to: 'login'
          }
        default:
          return {
            title: this.$t('lobby.password_remembered'),
            info: this.$t('lobby.or_login'),
            to: 'login'
          }
      }
