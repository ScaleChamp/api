<template>
  <ChartFrame :name="$t('metrics_comp.disk')">
    <canvas ref="disk"/>
  </ChartFrame>
</template>
...
export default {
  ...
  methods: {
    toMap (x) {
      return {
        ...
        data: x.data.disk_used.map(toMetric).sort((a, b) => b.x - a.x),
        borderWidth: 2
      }
    }
  },
  ...
  mounted () {
    this.disk = new Chart(this.$refs.disk, {
      type: 'line',
      data: {
  ...
  watch: {
    metrics (newVal) {
      this.disk.data.datasets = this.metrics.map(this.toMap)
      this.disk.update()
    }
  }
}
